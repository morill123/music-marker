...
function createTrack() {
    ...
    volumeSlider.addEventListener('input', () => {
        ...
        track.querySelectorAll('.clip').forEach(clip => {
            ...
            if (clip.audio) {
                clip.audio.volume = clip.volume * track.volume;
            }
        });
    });
    ...
    loopToggle.addEventListener('click', () => {
        ...
        isLooping = Array.from(document.querySelectorAll('.track')).some(t => t.isLooping);
    });
...
function updatePlayheadHeight() {
    ...
    const totalHeight = Array.from(tracks).reduce((sum, track) => {
        const style = window.getComputedStyle(track);
        const marginTop = parseInt(style.marginTop);
        const marginBottom = parseInt(style.marginBottom);
        return sum + track.offsetHeight + marginTop + marginBottom;
    }, 0);
    playhead.style.height = `${totalHeight}px`;

    // Ensure tracks-container's height is at least equal to the total track height
    tracksContainer.style.minHeight = `${totalHeight}px`;
}
...
function addMicRecordingToTrack(audioUrl) {
    const firstTrack = document.querySelector('.track') || createTrack();
    const newClip = createClip('mic-recording', playheadPosition);
    
    const audio = new Audio(audioUrl);
    audio.addEventListener('loadedmetadata', () => {
        const duration = audio.duration;
        const pixelsPerSecond = 20; // Adjust this value to match your timeline scale
        const width = duration * pixelsPerSecond;
        newClip.style.width = `${width}px`;
    });

    newClip.audio = audio;
    newClip.volume = 1; // Initialize clip volume
    audio.volume = firstTrack.volume || 1; // Set initial volume based on track volume

    firstTrack.querySelector('.track-content').appendChild(newClip);

    // Calculate and set the actual recording duration
    const recordingDuration = (Date.now() - micRecordStartTime) / 1000;
    const pixelsPerSecond = 20;
    const clipWidth = recordingDuration * pixelsPerSecond;
...
function createClip(instrumentType, left) {
    ...
    volumeSlider.addEventListener('input', () => {
        ...
        updateVolumeSlider(volumeSlider);
        if (audioSources[clip.id]) {
            const track = clip.closest('.track');
            const trackVolume = track.volume || 1;
            audioSources[clip.id].gainNode.gain.setValueAtTime(clip.volume * trackVolume, audioContext.currentTime);
        }
    });
    ...
    setTimeout(updatePlayheadHeight, 0);
    return clip;
}
...
function playTracks() {
    document.querySelectorAll('.track').forEach(track => {
        ...
        function playTrackClips(clipStartTime) {
            clips.forEach((clip) => {
                ...
                const clipStart = (leftPosition / trackWidth) * getTimelineDuration();
                const clipDuration = clip.audio ? clip.audio.duration : audioBuffers[instrumentType].duration;

                if (clipStart + clipDuration > clipStartTime) {
                    const delay = Math.max(0, clipStart - clipStartTime);

                    setTimeout(() => {
                        if (instrumentType === 'mic-recording') {
                            if (clip.audio) {
                                clip.audio.currentTime = 0;
                                clip.audio.volume = clip.volume * trackVolume;
                                clip.audio.play();
                            }
                        } else if (audioBuffers[instrumentType]) {
                            ...
                            source.start(audioContext.currentTime, 0, clipDuration);
                            audioSources[clip.id] = { source, gainNode };
                        }
                    }, delay * 1000);
                }

                trackDuration = Math.max(trackDuration, clipStart + clipDuration);
            });

            if (track.isLooping) {
                setTimeout(() => playTrackClips(0), trackDuration * 1000);
            }
        }
...
function updatePlayhead() {
    ...
    playheadPosition = 120 + (elapsedTime / getTimelineDuration()) * (tracksContainer.offsetWidth - 120);
    ...
    if (!isLooping) {
        ...
        tracks.forEach(track => {
            track.querySelectorAll('.clip').forEach(clip => {
                ...
                const clipDuration = clip.audio ? clip.audio.duration : audioBuffers[instrumentType].duration;
                maxEndTime = Math.max(maxEndTime, clipStartTime + clipDuration);
            });
        });
...
stopButton.addEventListener('click', () => {
    ...
    pausedTime = 0;
    isLooping = false;
    playheadPosition = 120; // Reset playhead position
    playhead.style.left = '120px'; // Update playhead visual position
    stopAudio();
    playButton.textContent = '▶️';
});
...
downloadButton.addEventListener('click', () => {
    if (recordedAudios.length > 0) {
        recordedAudios.forEach((audioUrl, index) => {
            ...
            a.href = audioUrl;
            a.download = `recorded-audio-${index + 1}.mp3`;
            a.click();
        });
    }else {
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Delete') {
        ...
        e.preventDefault();
        navigateClips(e.key === 'ArrowLeft' ? -1 : 1);
    }
});
...
function navigateClips(direction) {
    ...
    const clips = Array.from(selectedTrack.querySelectorAll('.clip'));
    if (clips.length === 0) return;

    let currentIndex = selectedClip ? clips.indexOf(selectedClip) : -1;
    currentIndex = (currentIndex + direction + clips.length) % clips.length;

    if (selectedClip) {
        selectedClip.classList.remove('selected');
    }
    selectedClip = clips[currentIndex];
